...
def main(argv: List[str] | None = None) -> int:
    ...
    with csv.open("w", encoding="utf-8") as f:
        f.write(
            "date,equity,daily_pnl,daily_return_pct,trades,max_drawdown_pct,sharpe\n"
        )
        prev = args.capital
        for i, e in enumerate(equity):
            ...
            f.write(
                f"{today},{e:.5f},{pnl:.5f},{ret:.6f},{trades},{mdd:.6f},{sharpe:.6f}\n"
            )
            prev = e
    ...
    Path("portfolio_snapshot.json").write_text(orjson.dumps(snap).decode("utf-8"))
    return 0
