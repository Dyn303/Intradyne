...
async def download_and_resample(
    exchange: str,
    symbol: str,
    start_date: str,
    end_date: str,
    ...
    logger.info(
        f"Downloading trades for {exchange} {tardis_symbol} from {start_date} to {end_date}..."
    )
    ...
    if not files:
        logger.error(
            "No files downloaded from Tardis. Check the dates or symbol format."
        )
        return
...
def main():
    parser = argparse.ArgumentParser(
        description="Download and resample data from Tardis.dev into Intradyne's format"
    )
    ...
    for symbol in symbols:
        asyncio.run(
            download_and_resample(
                args.exchange, symbol, args.start, args.end, args.timeframe, out_dir
            )
        )
